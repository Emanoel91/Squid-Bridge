...
# Chart 2
with col2:
    fig2 = px.area(df, x="Date", y="Retention Rate", title="%User Retention Rate Over Time")
    fig2.update_layout(
        xaxis_title=" ",
        yaxis_title="%",
        template="plotly_white"
...
SELECT created_at, source_chain, destination_chain, amount_usd
...
ORDER BY 1
    ...
    return pd.read_sql(query, conn)
# --- Load Data --------------------------------------------------------------
destination_chain_stats = load_destination_chain_stats(timeframe, start_date, end_date)
# --- stacked bar Chart ------------------------------------------------------
fig_stacked = px.bar(
    destination_chain_stats,
    x="Date",
    y="Amount (USD)",
    color="Chain",
    title="🟢Inflow to Chains Over Time"
)
fig_stacked.update_layout(barmode="stack", yaxis_title="$USD")
st.plotly_chart(fig_stacked, use_container_width=True)
...
fig_line = px.line(
    ...
    y="Total Amount (USD)",
    color="Chain",
    title="🟢Cumulative Inflow to Chains Over Time"
)
fig_line.update_layout(yaxis_title="$USD")
st.plotly_chart(fig_line, use_container_width=True)
...
st.markdown(
    ...
    <div style="margin-top: 20px; margin-bottom: 20px; font-size: 16px;">
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            ...
            <span>Dashboard Reference (Some of the code and charts were modified and rewritten): <a href="https://flipsidecrypto.xyz/Azin/squid-bridge-kHpcoT" target="_blank">https://flipsidecrypto.xyz/Azin/squid-bridge-kHpcoT/</a></span>
        </div>
        <div style="display: flex; align-items: center; gap: 10px;">
